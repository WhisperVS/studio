...
export const MobileAssetTable = React.memo(function MobileAssetTable({ assets, onEdit, onInfo, onDelete, selectedAssetIds, onSelectedAssetIdsChange, columnVisibility, tableHeight, isMobile = true }: AssetTableProps) {
  ...
  const getResponsiveClass = (defaultClass: string) => {
    return isMobile ? '' : defaultClass;
  };

  // Helper function to get mobile-appropriate column widths
  const getMobileColumnStyle = (columnId: string) => {
    if (!isMobile) return {};
    
    const mobileWidths: Record<string, string> = {
      category: '120px',
      status: '100px', 
      machineName: '140px',
      manufacturer: '120px',
    ...
    return { minWidth: mobileWidths[columnId] || '100px' };
  };
                  ...
                  <TableHeaderRow className="rounded-none">
                    ...
                    {APP_CONFIG.tableColumns.map(col => columnVisibility[col.id] && (
                        <TableHead 
                          ...
                          className={cn("cursor-pointer whitespace-nowrap h-16 text-base font-semibold text-center align-middle", isMobile ? "" : col.className)}
                          style={getMobileColumnStyle(col.id)}
                        >
                  ...
                  try {
                    return sortedAssets.map((asset) => (
                      <TableRow key={asset.id} data-state={selectedIdsSet.has(asset.id) ? "selected" : ""}>
                        ...
                        {columnVisibility.category && <TableCell style={getMobileColumnStyle('category')}>{getCategoryName(asset.category)}</TableCell>}
                        {columnVisibility.status && <TableCell style={getMobileColumnStyle('status')}><Badge variant={getStatusVariant(asset.status)}>{asset.status}</Badge></TableCell>}
                        {columnVisibility.machineName && <TableCell className="font-medium" style={getMobileColumnStyle('machineName')}>{asset.machineName}</TableCell>}
                        {columnVisibility.manufacturer && <TableCell className={getResponsiveClass("hidden md:table-cell")} style={getMobileColumnStyle('manufacturer')}>{asset.manufacturer}</TableCell>}
                        {columnVisibility.modelNumber && <TableCell className={getResponsiveClass("hidden lg:table-cell")} style={getMobileColumnStyle('modelNumber')}>{asset.modelNumber}</TableCell>}
                        {columnVisibility.type && <TableCell className={getResponsiveClass("hidden lg:table-cell")} style={getMobileColumnStyle('type')}>{asset.type}</TableCell>}
                        {columnVisibility.partNumber && <TableCell className={getResponsiveClass("hidden lg:table-cell")} style={getMobileColumnStyle('partNumber')}>{asset.partNumber}</TableCell>}
                        {columnVisibility.serialNumber && <TableCell className={getResponsiveClass("hidden xl:table-cell")} style={getMobileColumnStyle('serialNumber')}>{asset.serialNumber}</TableCell>}
                        {columnVisibility.os && <TableCell className={getResponsiveClass("hidden xl:table-cell")} style={getMobileColumnStyle('os')}>{asset.os}</TableCell>}
                        {columnVisibility.assignedUser && <TableCell style={getMobileColumnStyle('assignedUser')}>{asset.assignedUser || 'N/A'}</TableCell>}
                        {columnVisibility.userId && <TableCell className={getResponsiveClass("hidden sm:table-cell")} style={getMobileColumnStyle('userId')}>{asset.userId || 'N/A'}</TableCell>}
                        {columnVisibility.location && <TableCell className={getResponsiveClass("hidden 2xl:table-cell")} style={getMobileColumnStyle('location')}>{asset.location}</TableCell>}
