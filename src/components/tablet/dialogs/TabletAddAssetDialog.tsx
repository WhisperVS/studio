...
export function TabletAddAssetDialog({ isOpen, onOpenChange, onAssetAdded }: AddAssetDialogProps) {
  const { toast } = useToast();
  ...
  // Auto-fill part number for Dell
  ...
  // Scroll management for suggestions
  useEffect(() => {
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={(open: boolean) => {
        ...
        <DialogContent 
          className="w-[95vw] max-w-5xl max-h-[90vh] overflow-y-auto p-6 tablet-dialog-fix"
        ...
        >
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            <Button type="button" variant="outline" size="default" onClick={() => setIsJsonImportOpen(true)} className="shadow-sm">
              <FileJson className="mr-2 h-4 w-4" />
              Import from JSON
            </Button>
            <Button type="button" variant="outline" size="default" onClick={() => setIsCommandDialogOpen(true)} className="shadow-sm">
              <ClipboardCopy className="mr-2 h-4 w-4" />
              Copy Info Script
            </Button>
            <Button type="button" variant="destructive" size="default" onClick={handleClearForm} className="ml-auto shadow-sm">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 pt-4 tablet-form-spacing" autoComplete="off">
              <FormField
                control={form.control}
                name="owner"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel htmlFor="owner-input" className="text-base">{APP_CONFIG.labels.owner}</FormLabel>
                    <FormControl>
                      <Input {...field} id="owner-input" name="owner" readOnly className="bg-muted h-11" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-4 items-start">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="machine-name-input" className="text-base">{APP_CONFIG.labels.machineName}</FormLabel>
                      <FormControl>
                        <Input placeholder="e.g., WKSTN-DEV-01" {...field} id="machine-name-input" name="machineName" value={field.value ?? ''} className="h-11" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="category-select" className="text-base">{APP_CONFIG.labels.category}</FormLabel>
                      <Select onValueChange={(v: string | undefined) => field.onChange(v || undefined)} value={field.value ?? undefined}>
                        <FormControl>
                          <SelectTrigger id="category-select" name="category" className="h-11">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-base">{APP_CONFIG.labels.manufacturer}</FormLabel>
                      <FormControl>
                        <Combobox
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="location-select" className="text-base">{APP_CONFIG.labels.location}</FormLabel>
                      <Select onValueChange={(v: string | undefined) => field.onChange(v || undefined)} value={field.value ?? undefined}>
                        <FormControl>
                          <SelectTrigger id="location-select" name="location" className="h-11">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="model-number-input" className="text-base">{APP_CONFIG.labels.modelNumber}</FormLabel>
                      <FormControl>
                        <div className="relative">
                          ...
                          <div className="absolute z-50 mt-1 w-full rounded-md bg-popover text-popover-foreground shadow-lg border">
                            <ul
                            ...
                            >
                              {modelSuggestions.map((s, i) => (
                                <li
                                  ...
                                  className={`px-3 py-3 text-sm cursor-pointer transition-colors hover:bg-accent hover:text-accent-foreground ${i === activeModelSuggestionIndex ? 'bg-accent text-accent-foreground' : ''}`}
                                >
                                  {s}
                                </li>
                              ))}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="part-number-input" className="text-base">{APP_CONFIG.labels.partNumber}</FormLabel>
                      <FormControl>
                        <Input
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="serial-number-input" className="text-base">{APP_CONFIG.labels.serialNumber}</FormLabel>
                      <FormControl>
                        <Input
                ...
                {category && !['printers', 'networks', 'misc'].includes(category) && (
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor="os-input" className="text-base">{APP_CONFIG.labels.os}</FormLabel>
                        <FormControl>
                          <div className="relative">
                            <Input
                              aria-autocomplete="list"
                              ...
                              >
                                {osSuggestions.map((s, i) => (
                                  <li
                                    ...
                                    className={`px-3 py-3 text-sm cursor-pointer transition-colors hover:bg-accent hover:text-accent-foreground ${i === activeOsSuggestionIndex ? 'bg-accent text-accent-foreground' : ''}`}
                                  >
                                    {s}
                                  </li>
                                ))}
                ...
                {category && (category === 'systems' || category === 'servers') && (
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor="type-select" className="text-base">{APP_CONFIG.labels.type}</FormLabel>
                        <Select onValueChange={field.onChange} value={field.value ?? undefined}>
                          <FormControl>
                            <SelectTrigger id="type-select" name="type" className="h-11">
                ...
                {showWebUI && (
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor="webui-input" className="text-base">{APP_CONFIG.labels.webui}</FormLabel>
                        <FormControl>
                          <div className="flex">
                            <Select
                            ...
                            >
                              <SelectTrigger className="w-32 rounded-r-none border-r-0 h-11">
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="status-select" className="text-base">{APP_CONFIG.labels.status}</FormLabel>
                        <Select onValueChange={(v: string | undefined) => field.onChange(v || undefined)} value={field.value ?? undefined}>
                        <FormControl>
                          <SelectTrigger id="status-select" name="status" className="h-11">
              ...
              {status && !['For Recycle', 'For Parts'].includes(status) && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-4 p-6 border rounded-lg bg-card items-start">
                  <div className="lg:col-span-2">
                    <p className="font-medium text-base text-foreground">User Assignment</p>
                  </div>
                  <FormField
                    control={form.control}
                    name="assignedUser"
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-base">{APP_CONFIG.labels.assignedUser}</FormLabel>
                        <div className="relative">
                          <FormControl>
                            <Input
                              aria-autocomplete="list"
                              ...
                              >
                                {userSuggestions.map((s, i) => (
                                  <li
                                    ...
                                    className={`px-3 py-3 text-sm cursor-pointer ${i === activeUserSuggestionIndex ? 'bg-accent text-accent-foreground' : ''}`}
                                  >
                                    {s}
                                  </li>
                                ))}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel htmlFor="user-id-input" className="text-base">{APP_CONFIG.labels.userId}</FormLabel>
                        <div className="relative">
                          <FormControl>
                            <Input type="text" placeholder="e.g., 0005" {...field} id="user-id-input" name="userId" value={field.value ?? ''} className="h-11" />
                          </FormControl>
                        </div>
                        <FormMessage />
                      </FormItem>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="space-y-2 pt-2">
                          <FormLabel className="text-base">{APP_CONFIG.labels.userType}</FormLabel>
                          <FormControl>
                            <RadioGroup
                            ...
                            >
                              <FormItem className="flex items-center space-x-3 space-y-0">
                                <FormControl>
                                  <RadioGroupItem value="local" className="w-5 h-5" />
                                </FormControl>
                                <FormLabel className="font-normal text-base">Local</FormLabel>
                              </FormItem>
                              <FormItem className="flex items-center space-x-3 space-y-0">
                                <FormControl>
                                  <RadioGroupItem value="remote" className="w-5 h-5" />
                                </FormControl>
                                <FormLabel className="font-normal text-base">Remote</FormLabel>
                              </FormItem>
                            </RadioGroup>
                          </FormControl>
                        </FormItem>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-4 items-start">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      <FormLabel htmlFor="purchase-date-picker" className="text-base">{APP_CONFIG.labels.purchaseDate}</FormLabel>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-col">
                      <FormLabel htmlFor="warranty-date-picker" className="text-base">{APP_CONFIG.labels.warrantyExpirationDate}</FormLabel>
                      <DatePicker date={field.value ?? undefined} setDate={(d) => field.onChange(d)} />
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel htmlFor="notes-textarea" className="text-base">{APP_CONFIG.labels.notes}</FormLabel>
                    <FormControl>
                      <Textarea
              ...
              <DialogFooter className="pt-6 flex-row justify-end items-center gap-3">
                <Button type="button" variant="outline" onClick={() => onOpenChange(false)} className="h-11 px-6">
                  Cancel
                </Button>
                <Button type="submit" variant="primary" className="h-11 px-6">Add Asset</Button>
              </DialogFooter>
            </form>
          </Form>
        </DialogContent>
