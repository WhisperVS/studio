...
export function DesktopAddAssetDialog({ isOpen, onOpenChange, onAssetAdded }: AddAssetDialogProps) {
  ...
  <Dialog open={isOpen} onOpenChange={(open: boolean) => {
      ...
      <DialogContent 
        className="w-[95vw] max-w-4xl max-h-[90vh] overflow-y-auto p-4 sm:p-6 mobile-dialog-fix"
  onInteractOutside={(e: React.SyntheticEvent) => {
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1">
            <Button type="button" variant="outline" size="sm" onClick={() => setIsJsonImportOpen(true)} className="shadow-sm flex-shrink-0">
              <FileJson className="mr-2 h-4 w-4" />
              <span className="hidden sm:inline">Import from JSON</span>
              <span className="sm:hidden">Import</span>
            </Button>
            <Button type="button" variant="outline" size="sm" onClick={() => setIsCommandDialogOpen(true)} className="shadow-sm flex-shrink-0">
              <ClipboardCopy className="mr-2 h-4 w-4" />
              <span className="hidden sm:inline">Copy Info Script</span>
              <span className="sm:hidden">Copy</span>
            </Button>
            <Button type="button" variant="destructive" size="sm" onClick={handleClearForm} className="ml-auto shadow-sm flex-shrink-0">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-2 mobile-form-spacing" autoComplete="off">
